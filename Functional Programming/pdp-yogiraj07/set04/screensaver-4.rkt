...
(define  (world-to-scene w)
  (place-images (append (draw-rect-list (world-rects w))
                ...
                (append (draw-posn-list (world-rects w) (world-state-mx w)
                        (world-state-my w)) (dot-posn-list (world-rects w)))
                (draw-canvas w))) 
...
;;DESIGN STRATEGY: Use template for LOR on lor
...
(define (draw-dot-list lor)
  (cond
    [(empty? lor) empty]
    [else
     (append (get-dot-images (rectl-lod (first lor)))
             (draw-dot-list(rest lor)))]))
...
(define (get-dot-images lod)
  (cond
    [(empty? lod) (cons WCIRCLE '())]
    [else
     (cons DOT (get-dot-images(rest lod)))]))

;;dot-posn-list: LOR-> List of dot positions
;;GIVEN: LOR lor, whose dots needs to be printed
...
(define (dot-posn-list lor)
  (cond
    [(empty? lor) empty]
    [else
     (append (get-dot-posn-list (rectl-lod (first lor)))
             (dot-posn-list(rest lor)))]))
...
(define (get-dot-posn-list lod)
  (cond
    [(empty? lod) (cons (make-posn 0 0) '())]
    [else
     (cons (get-dot-posn (first lod)) (get-dot-posn-list(rest lod)))]))
...
;;DESIGN STRATEGY: Use cases on whether lor is empty
...
;;check-selected? : LOR-> Boolean
;;GIVEN: LOR
;;RETURNS: True iff any one Rectangle in LOR is selected
;;DESIGN STRATEGY : use case on whether Rectangle is selected
(define (check-selected? lor)
...
;;GIVEN: A List of Rectangles lor from the current WolrdState
