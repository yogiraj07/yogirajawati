...
public class FrmMain extends javax.swing.JFrame implements MouseListener, MouseMotionListener {
    ...
    public FrmMain(FrmSettings parent) {
        ...
        parent.ImageTransform(ww, hh); //to upload default image of snapshot
        byte[] myArrray;
        int count = 0;
        ...
        if (!parent.currDevice.service.equals("Snapshot")) { //code for video feed
            cw = 400;
            ch = 300;
        ...
        try {
            ...
            if (rs.next()) {
                ...
                pre.setBinaryStream(1, bis, (long) (myArrray.length));// 1 denotes first question mark
                pre.executeUpdate();
...
//            System.out.println("Database Connection OK");
    ...
    public void updateImage() {
        ...
        if (connect) { //if connect button is being pressed
            Calendar cal = Calendar.getInstance();
            try {
    ...
    private void jBLoadNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBLoadNewActionPerformed
        if (parent.currDevice.service.equals("Snapshot")) { //default image uploaded
            jLabelFeed.setIcon(new ImageIcon(parent.scSmall));
            return;
        }
        if (parent.flag) {    //explicitly uploaded
            jLabelFeed.setIcon(new ImageIcon(parent.scSmall));
            return;
        }
