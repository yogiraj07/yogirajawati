...
public class MyService {
    ...
    @WebMethod(operationName = "StreamVideo")   //returns image in pixel format ....converts from bytes to pixel
    public MyImage StreamVideo(@WebParam(name = "eIn") int eIn) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                byte[] temp;
                temp = b.getBytes(1, (int) b.length()); //blob to bytes conversion
...
//                }
                ...
                for (int y = 0; y < mi.hh; y++) {
                    for (int x = 0; x < mi.ww; x++) {
                        int col = (temp[cnt] & 0xff) << 16; //R value goes to 16th position of integer 4-byte col
                        cnt++;
                        col |= (temp[cnt] & 0xff) << 8; //value is assigned then shifted
                        cnt++;
                        col |= (temp[cnt] & 0xff);
                        cnt++;
                        mi.img[k] = col; 
    ...
    @WebMethod(operationName = "GetAllDevices") //it returns available devices in the network
