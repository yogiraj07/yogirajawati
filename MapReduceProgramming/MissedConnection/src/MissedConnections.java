...
public class MissedConnections {
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job(config, "Missed Connections");
		...
		public void map(Object offset, Text value, Context context) throws IOException, InterruptedException {
			...
			try {
				if (record.length > 0 && isRecordValid(record) ) {
					...
					carrierName.set(record[6].getBytes()); 
					year.set(record[0].getBytes()); 
					originId.set(record[11].getBytes());
					destinationId.set(record[20].getBytes());
					date.set(record[5].getBytes());
					arrivalTime.set(record[41].getBytes());
					scheduledArrivalTime.set(record[40].getBytes());
					deptTime.set(record[30].getBytes());
					scheduleddeptTime.set(record[29].getBytes());
					cancellation.set(record[47].getBytes());
